...
class TestCollection extends EventEmitter {
	...
	_buildHook(hook, testTitle, context) {
		...
		const test = new Test(hook.metadata, title, hook.fn, false, context, this._emitTestResult);
	...
	_buildTest(test, context) {
		...
		test = new Test(test.metadata, test.title, test.fn, this.failWithoutAssertions, context, this._emitTestResult);
		this.pendingTestInstances.add(test);
		return test;
	}
